...
lo=x_tmp;
load(strcat('/HeLabData2/cxpang/DIDA/',method,'/result_xy/brain_age/validation.mat'),'percent','adjusted_mdd_all_gap_center')
...
for s=1:10
 ...
 load ('/HeLabData2/cxpang/DIDA/using_mat/site_remove65.mat')%age,group,mfd,sex
  ...
  pos_center=find(site~=s);
 
    pos_hc=intersect(pos_center,find(group==1));
        pos_mdd=intersect(pos_center,find(group==2));

  sub1_index=pos_mdd(find(adjusted_mdd_all_gap_center{s}<0));
    sub2_index=pos_mdd(find(adjusted_mdd_all_gap_center{s}>0));
    ...
    vars_loso('all','ot1psighc', hc_weight,hc_sex, hc_age, hc_mfd,s)
    vars_loso('all','ot1psigsub1', sub1_weight,mdd1_sex, mdd1_age, mdd1_mfd,s)
    ...
    if(min(weight_pca_neg)<0)
        ...
        hc_weight=weight_new(pos_hc);
sub1_weight=weight_new(sub1_index);
    vars_loso('all','ot1npsighc', hc_weight,hc_sex, hc_age, hc_mfd,s)
    vars_loso('all','ot1npsigsub1', sub1_weight,mdd1_sex, mdd1_age, mdd1_mfd,s)
    ...
    vars_loso('all','ot2psighc', hc_weight,hc_sex, hc_age, hc_mfd,s)
    vars_loso('all','ot2psigsub2', sub2_weight,mdd2_sex, mdd2_age, mdd2_mfd,s)
    ...
    if(min(weight_pca_neg2)<0)
        ...
        hc_weight=weight_new(pos_hc);
    sub2_weight=weight_new(sub2_index);
    vars_loso('all','ot2npsighc', hc_weight,hc_sex, hc_age, hc_mfd,s)
    vars_loso('all','ot2npsigsub2', sub2_weight,mdd2_sex, mdd2_age, mdd2_mfd,s)
...
for s=1:10
    ...
    yfitM=nan(1000,5);
    load(strcat(inpath2,...
        'GAMLSS_','t1asighc_',num2str(s),'_predicted_sex0_WHOLE_SAMPLE.mat'),'predictions_quantiles','age');
    yfitF=predictions_quantiles; %estimated females at site i (assumes females = 0)
    faf_age=age;
    load(strcat(inpath2,...
        'GAMLSS_','t1asighc_',num2str(s),'_predicted_sex1_WHOLE_SAMPLE.mat'),'predictions_quantiles','age');
    ...
    % gap>0
    load(strcat(inpath2,...
        'GAMLSS_','t1asigsub1_',num2str(s),'_predicted_sex0_WHOLE_SAMPLE.mat'),'predictions_quantiles','age');
    yfitF=predictions_quantiles; %estimated females at site i (assumes females = 0)
    faf_age=age;
    load(strcat(inpath2,...
        'GAMLSS_','t1asigsub1_',num2str(s),'_predicted_sex1_WHOLE_SAMPLE.mat'),'predictions_quantiles','age');
    yfitM=predictions_quantiles; %estimated males at site i (assumes males = 1)
    fam_age=age;
    ...
    yfitM=nan(1000,5);
    load(strcat(inpath2,...
        'GAMLSS_','t2asighc_',num2str(s),'_predicted_sex0_WHOLE_SAMPLE.mat'),'predictions_quantiles','age');
    yfitF=predictions_quantiles; %estimated females at site i (assumes females = 0)
    faf_age=age;
    load(strcat(inpath2,...
        'GAMLSS_','t2asighc_',num2str(s),'_predicted_sex1_WHOLE_SAMPLE.mat'),'predictions_quantiles','age');
    yfitM=predictions_quantiles; %estimated males at site i (assumes males = 1)
    fam_age=age;
    yfit=(yfitF+yfitM)/2;
    fake_age=(faf_age+fam_age)/2;
    ...
    % gap>0
    load(strcat(inpath2,...
        'GAMLSS_','t2asigsub2_',num2str(s),'_predicted_sex0_WHOLE_SAMPLE.mat'),'predictions_quantiles','age');
    yfitF=predictions_quantiles; %estimated females at site i (assumes females = 0)
    faf_age=age;
    load(strcat(inpath2,...
        'GAMLSS_','t2asigsub2_',num2str(s),'_predicted_sex1_WHOLE_SAMPLE.mat'),'predictions_quantiles','age');
    yfitM=predictions_quantiles; %estimated males at site i (assumes males = 1)
    fam_age=age;
    yfit_pos=(yfitF+yfitM)/2;
    fake_age_pos=(faf_age+fam_age)/2;
