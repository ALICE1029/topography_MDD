...
%%
%% step3 select feature for time saving
ResultsFolder = '/HeLabData2/cxpang/DIDA/NGSR/result_xy/hamd';
matFolder='/home/cxpang/matlab/code/8.brain_age/using_mat';

fold=10
FoldQuantity = 10;
select_p_all=[0.05,0.01,0.001];
for j=1:3
select_p=select_p_all(j);
ResultantFolder = strcat('/HeLabData2/cxpang/DIDA/NGSR/result_xy/brain_age/sub1/p',num2str(select_p))

for i=1:fold
    i
    load(strcat('/HeLabData2/cxpang/DIDA/NGSR/result_xy/brain_age/sub1/Fold_',num2str(i-1),'_regress_remove65.mat'))
    load(strcat('/HeLabData2/cxpang/DIDA/NGSR/result_xy/brain_age/sub1/Fold_',num2str(i-1),'_regress_score_remove65site.mat'))
    [my_corr,p]=corr(Subjects_Data_train,Subjects_Score_train,'type','Pearson');
    pos = find(any(p <select_p, 2));
    pos=pos-1;    
    save(strcat(ResultantFolder,'/fold_',num2str(i-1),'_mask_remove65site.mat'),'pos')
end
end
%%
ResultsFolder = '/HeLabData2/cxpang/DIDA/NGSR/result_xy/hamd';
...
for j=1:3
select_p=select_p_all(j);
ResultantFolder = strcat('/HeLabData2/cxpang/DIDA/NGSR/result_xy/brain_age/sub2/p',num2str(select_p))

for i=1:fold
    i
    load(strcat('/HeLabData2/cxpang/DIDA/NGSR/result_xy/brain_age/sub2/Fold_',num2str(i-1),'_regress_remove65.mat'))
    load(strcat('/HeLabData2/cxpang/DIDA/NGSR/result_xy/brain_age/sub2/Fold_',num2str(i-1),'_regress_score_remove65site.mat'))
    [my_corr,p]=corr(Subjects_Data_train,Subjects_Score_train,'type','Pearson');
    pos = find(any(p <select_p, 2));
    pos=pos-1;    
    save(strcat(ResultantFolder,'/fold_',num2str(i-1),'_mask_remove65site.mat'),'pos')
...
for k=1:1000
    ...
    if(exist(strcat(ResultantFolder,'Fold_9comp_uv.mat')))
        for  s=1
             draw=cell(2,1);
            for i=0:9%to avoid the condition that predict value same,correlation nan
                %       if(max(Prediction.Score{i})-min(Prediction.Score{i})==0)
                %           Prediction.Corr(i)=0;
                %       end
                %
                ResultantFolder = [strcat('/HeLabData2/cxpang/DIDA/',method,'/result_xy/hamd/cca/AtlasLoading/loading_sub_sub1_select0.001/Permutation/',num2str(k)),'/Time_0/'];
