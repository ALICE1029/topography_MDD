...
def PLSr1_KFold_RandomCV(select_p,Subjects_Data, Subjects_Score, Covariates, Fold_Quantity, ComponentNumber_Range,
    ...
    for j in np.arange(Fold_Quantity):# ten-cv
        ...
        if j <= 9 and select_p<1:          
           item_Quantity = np.shape(Subjects_Score_train)[1];
           for k in np.arange(Features_Quantity):
                for s in np.arange(item_Quantity):
                    r,p=pearsonr( Subjects_Data_train[:, k],  Subjects_Score_train[:,s])
                    if p<select_p:
                          mask.append(k)
           mask = np.unique(mask)
           b=[]
