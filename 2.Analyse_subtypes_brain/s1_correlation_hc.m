...
load('/home/cxpang/matlab/code/8.brain_age/index_hc_remove65sex.mat','Idx')
...
sub_gap(hc)=ad_gap_new_sort;
sub1_index=hc(find(Idx==1));
sub2_index=hc(find(Idx==2));
...
save('/HeLabData2/cxpang/DIDA/NGSR/result_xy/brain_age/loading_hc_remove65.mat','loading_sub1','loading_sub2');
%% sub1
...
for i = 1:network_num
    i
    for j = 1:n_voxel
        my_corr2(i,j)=corr(loading_sub1{i}(:,j),abs(sub1_gap));
    end
end
...
save(strcat('/HeLabData2/cxpang/DIDA/',method,'/result_xy/betweengroup_difference/corr_hc_1_remove65sex.mat'),'T_value','p_value','res_map','dfe');
...
for i =1:18% 1:network_num
   ...
   % tmp(pos)=[];
        ...
        if length(find(theCurrentCluster)) <= cluster_size(i)
          ...
          %  Z{i}(j)= max(abs(Z_vol{i}(find(theCurrentCluster)))) ;
            ...
            if ~isempty(max_positive)
             ...
             % t= max(abs(t_vol{i} (find(theCurrentCluster)))) ;
            % cd{i}(j)= t*(sqrt(1/length(hc1_index)+1/length(sub1_index)));
            
            % t= min(abs(t_vol{i} (find(theCurrentCluster)))) ;
            
            % cd_min{i}(j)= t*(sqrt(1/length(hc1_index)+1/length(sub1_index)));
    ...
    z_grf{i}=Z_vol{i}(ind);
    t_grf{i}=t_vol{i}(ind);
    outDir = strcat ('/HeLabData2/cxpang/DIDA/',method,'/result_xy/corr_hc_1_remove65sexmask');
    saveFig = 0;
...
dfe=stat_result.tstat.dfe;
save(strcat('/HeLabData2/cxpang/DIDA/',method,'/result_xy/betweengroup_difference/corr_hc_2_remove65sex.mat'));
...
for i = 1:network_num
    ...
    z_grf{i}=Z_vol{i}(ind);
    t_grf{i}=t_vol{i}(ind);
    outDir = strcat ('/HeLabData2/cxpang/DIDA/',method,'/result_xy/corr_hc_2_remove65sexmask');
    saveFig = 0;
     maskName ='/HeLabData2/cxpang//fmri/mask/GMMask_3mm.nii';
    maskNii = load_untouch_nii(maskName);
    if ~exist(outDir,'dir')
...
save_untouch_nii(kNii,outName);
%% combine sub 2
load(strcat('/HeLabData2/cxpang/DIDA/',method,'/result_xy/betweengroup_difference/corr_hc_2_grf_remove65sexmask.mat'),'t_grf')
t_combine=zeros(voxel_num,1);
t=zeros(network_num,voxel_num);
for i = 1:network_num
    i
